...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.6; color: #111827; max-width: 600px; margin: 0 auto; padding: 20px;">
  ...
  <div style="background: white; padding: 32px; border: 1px solid #E5E7EB; border-top: none; border-radius: 0 0 12px 12px;">
    <p style="font-size: 16px; margin-bottom: 16px;">Hi ${data.organisationName},</p>
    <p style="font-size: 16px; margin-bottom: 16px;">
      We weren't able to process your payment of <strong>${(data.currency as string)?.toUpperCase() || 'USD'} ${((data.amount as number) / 100).toFixed(2)}</strong>
    ...
    <div style="background: #F9FAFB; padding: 20px; border-radius: 8px; margin-bottom: 24px;">
      <table style="width: 100%; border-collapse: collapse;">
        <tr>
          ...
          <td style="padding: 8px 0; text-align: right; font-weight: 600;">${(data.currency as string)?.toUpperCase() || 'USD'} ${((data.amount as number) / 100).toFixed(2)}</td>
        </tr>
        <tr>
...
export const sendEmail = onCall<EmailRequest>({ region: 'europe-west2' }, async (request) => {
  ...
  if (!type || !to || !emailData) {
    throw new HttpsError(
      'invalid-argument',
      'Missing required fields: type, to, data'
    );
  }
...
export const sendEmailHttp = onRequest({ cors: true, region: 'europe-west2' }, async (req, res) => {
  ...
  try {
    const { type, to, data: emailData, branding } = req.body as EmailRequest;
