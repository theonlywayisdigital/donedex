...
async function handleCheckoutComplete(event: Stripe.Event) {
  ...
  if (session.metadata?.add_on_type === 'storage') {
    ...
    if (existingAddon.empty) {
      await db.collection('storage_addons').add({
        ...addonData,
        created_at: new Date().toISOString(),
      });
    } else {
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
  ...
  <div style="background: white; padding: 32px; border: 1px solid #E5E7EB; border-radius: 0 0 12px 12px;">
    ...
    await sendEmailWithResend(org.contact_email, 'Your Donedex subscription has been canceled', emailHtml);
  }
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
  ...
  <div style="background: white; padding: 32px; border: 1px solid #E5E7EB; border-radius: 0 0 12px 12px;">
    ...
    await sendEmailWithResend(org.contact_email, `Your Donedex trial ends in 3 days`, emailHtml);
  ...
  <div style="background: white; padding: 32px; border: 1px solid #E5E7EB; border-radius: 0 0 12px 12px;">
    ...
    <div style="background: #F9FAFB; padding: 20px; border-radius: 8px; margin: 24px 0;">
      <table style="width: 100%;">
        <tr><td style="color: #6B7280;">Amount Paid</td><td style="text-align: right; font-weight: 600;">${invoice.currency?.toUpperCase() || 'GBP'} ${(invoice.amount_paid / 100).toFixed(2)}</td></tr>
    ...
    ${invoice.invoice_pdf ? `<div style="text-align: center;"><a href="${invoice.invoice_pdf}" style="display: inline-block; background: ${primaryColor}; color: white; padding: 14px 32px; text-decoration: none; border-radius: 8px;">Download Invoice PDF</a></div>` : ''}
        ...
        await sendEmailWithResend(emailTo, 'Receipt for your Donedex payment', emailHtml);
      }
    }
  }
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
  ...
  <div style="background: white; padding: 32px; border: 1px solid #E5E7EB; border-radius: 0 0 12px 12px;">
    <p>Hi ${org.name},</p>
    <p>We weren't able to process your payment of <strong>${invoice.currency?.toUpperCase() || 'GBP'} ${(invoice.amount_due / 100).toFixed(2)}</strong>.</p>
    ...
    <div style="text-align: center; margin: 32px 0;">
      <a href="https://app.donedex.com/settings/billing" style="display: inline-block; background: ${primaryColor}; color: white; padding: 14px 32px; text-decoration: none; border-radius: 8px;">Update Payment Method</a>
        ...
        await sendEmailWithResend(emailTo, 'Action required: Payment failed', emailHtml);
      }
    }
  }
...
export const stripeWebhook = onRequest({ region: 'europe-west2', secrets: ['STRIPE_SECRET_KEY', 'STRIPE_WEBHOOK_SECRET', 'RESEND_API_KEY'] }, async (req, res) => {
