...
function determineSuggestedStep(
  ...
  if (hasTemplate) return 'generation';
...
export const templateBuilderChat = onCall<ChatRequest>({ region: 'europe-west2', secrets: ['GEMINI_API_KEY'] }, async (request) => {
...
// HTTP version
export const templateBuilderChatHttp = onRequest({ cors: true, region: 'europe-west2', secrets: ['GEMINI_API_KEY'] }, async (req, res) => {
