...
import type { Record as RecordModel, RecordType, Template, Report } from '../types';
import {
...
export async function fetchRecordsWithRecordType(): Promise<RecordsWithRecordTypeResult> {
  ...
  if (error) {
    console.error('Error fetching records with record type:', error);
    return { data: [], error: { message: error.message } };
  }
...
export async function fetchRecordsPaginated(
  ...
  if (error) {
    console.error('[Records] Error fetching paginated records:', error);
...
export async function searchRecords(
  ...
  if (error) {
    console.error('[Records] Error searching records:', error);
    return { data: [], error: { message: error.message } };
  }
...
export async function fetchRecordWithType(
  ...
  if (error) {
    console.error('[Records] Error fetching record with type:', error);
    return { data: null, error: { message: error.message } };
  }
...
export async function fetchRecordReportsSummary(
  ...
  if (error) {
    console.error('[Records] Error fetching record reports:', error);
    return { data: [], error: { message: error.message } };
  }
...
export async function fetchRecordReportsPaginated(
  ...
  if (error) {
    console.error('[Records] Error fetching paginated reports:', error);
    return emptyPaginatedResult();
  }
