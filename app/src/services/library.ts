...
export async function fetchLibraryRecordTypes(): Promise<LibraryRecordTypesResult> {
  ...
  if (error) {
    console.error('Error fetching library record types:', error);
...
export async function fetchLibraryRecordTypeById(
  ...
  if (error) {
    console.error('Error fetching library record type:', error);
...
export async function copyLibraryTemplateToOrg(
  ...
  if (templateError || !newTemplate) {
    console.error('Error creating template from library:', templateError);
  ...
  for (let sectionIndex = 0; sectionIndex < librarySections.length; sectionIndex++) {
    ...
    if (sectionError || !newSection) {
      console.error('Error creating template section:', sectionError);
...
export async function fetchLibraryRecordTypesWithTemplates(): Promise<{
  ...
  if (error) {
    console.error('Error fetching library record types with templates:', error);
    return { data: [], error: { message: error.message } };
  }
